...
import { fetchStadiumList } from "../../app/usecases/stadium/fetchStadiumList";
...
export const stadiumController = async (req:Request,res:Response)=>{
    const {stadiumname,maxcapacity,sportstype,fromdate,todate,price,image,discription,location,email} = req.body

    try{
        const Stadium = addStadium(stadiumRepository)(stadiumname,maxcapacity,sportstype,fromdate,todate,price,image,discription,location,email)
...
export const stadimDataFetch = async (req:Request,res:Response)=>{
    try {
        ...
        if(fetchStadiumData){
            res.status(200).json({sucess : "data fetch successfull",fetchStadiumData})
        }else{
